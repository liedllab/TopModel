"""Tests for the math part needed to determine the chirality"""
...
def test_distance_remain_same(values: np.ndarray, zero_point: np.ndarray) -> None:
...
def test_vector_zero(vector: np.ndarray) -> None:
...
def test_vector_zero_sign(vector: np.ndarray) -> None:
...
def test_theta_normalization(vector: np.ndarray) -> None:
    """Theta should be non negative and not higher than 2pi"""
...
def test_determinant_of_transformation(vector: np.ndarray) -> None:
    """The transformation matrix should still have the same relative orientation of i_hat, j_hat 
    and k_hat"""
    assume( not np.allclose(vector, 0) )
    matrix = get_transform(vector)
    matrix = get_transform(vector)
    assert np.sign(np.linalg.det(matrix)) == 1
